%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{P~R~O~J~E~C~T~S~~\&~~P~U~B~L~I~C~A~T~I~O~N~S~~}


%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------

\begin{cventries}
    \cventry
        {\href{https://github.com/PierpaoloV/social-agent}{github.com/PierpaoloV/social-agent}}
        {Social Agent}
        {Personal Project}
        {2026 -- Present}
        {Human-in-the-loop social agent (Python, OpenAI API): captures private Telegram notes and GitHub milestones, ranks ideas, drafts post options, routes approvals via Telegram, and schedules publishing with a separate private state store and tests.}
    \vspace{1mm}
    \cventry
        {\href{https://github.com/PierpaoloV/weekly\_ai\_paper}{github.com/PierpaoloV/weekly\_ai\_paper}}
        {Weekly AI Paper-Triage Agent}
        {Personal Project}
        {2024 -- Present}
        {Multi-agent pipeline (Anthropic API, Python): weekly arXiv / PubMed scan $\rightarrow$ LLM summarisation $\rightarrow$ interest-based scoring (0--10) $\rightarrow$ automated email digest. In active personal use since 2024.}
    \vspace{1mm}
    \cventry
        {\href{https://github.com/PierpaoloV/job-agent}{github.com/PierpaoloV/job-agent}}
        {AI Job-Search Agent}
        {Personal Project}
        {2025 -- Present}
        {End-to-end Python pipeline: Gmail API ingestion of job-alert emails $\rightarrow$ structured parsing (LinkedIn, Indeed, Glassdoor) $\rightarrow$ SQLite deduplication $\rightarrow$ LLM ranking with prompt caching (Claude Haiku) $\rightarrow$ Telegram digest. Runs 3$\times$/day via GitHub Actions.}
    \vspace{1mm}
    \cventry
        {\href{https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0301969}{doi:10.1371/journal.pone.0301969}}
        {Automatic quantification of tumor-stroma ratio as a prognostic biomarker for pancreatic cancer}
        {PLOS ONE}
        {May 2024}
        {Multi-class U-Net for TSR segmentation across 368 patients / 4 datasets (25 NL labs). Dice 0.75 (epithelium) and 0.86 (tumor bulk) on external TCGA-PAAD validation. TSR AUC 0.61 for 6-month survival prediction on independent cohort.}
    \vspace{1mm}
    \cventry
        {\href{https://openreview.net/pdf?id=Dtz_iaUpGc}{openreview.net/pdf?id=Dtz\_iaUpGc}}
        {Automatic quantification of TSR as a prognostic marker for pancreatic cancer}
        {MIDL 2023}
        {Sep. 2023}
        {Poster presentation: segmentation of tumor epithelium, healthy epithelium and TSR estimation from pancreatic WSIs.}
    \vspace{1mm}
    \cventry
        {\href{https://www.mdpi.com/2072-6694/14/14/3498}{doi:10.3390/cancers14143498}}
        {Setting the research agenda for clinical AI in pancreatic adenocarcinoma imaging}
        {Cancers}
        {Jul. 2022}
        {Systematic review on the current role of AI for pancreatic cancer diagnosis and personalised treatment.}
    \vspace{1mm}
    \cventry
        {\href{https://www.spiedigitallibrary.org/conference-proceedings-of-spie/12039/120391F/Automatic-tumour-segmentation-in-HE-stained-whole-slide-images-of/10.1117/12.2611542.short}{doi:10.1117/12.2611542}}
        {Automatic tumour segmentation in H\&E-stained whole-slide images of the pancreas}
        {SPIE MI 2022}
        {Apr. 2022}
        {Multi-task U-Net with auxiliary classification head for coarse pancreatic tumour area segmentation in WSIs; classification head reduced false positives.}
    \vspace{1mm}
    \cventry
        {}
        {End-to-end prognostication in pancreatic cancer by multimodal deep learning: a retrospective, multicenter study}
        {European Radiology}
        {2025}
        {Schuurmans et al. Multimodal deep learning combining CT imaging and clinical data for PDAC outcome prediction across multiple centers.}
    \vspace{1mm}
    \cventry
        {}
        {Artificial intelligence and radiologists in pancreatic cancer detection (PANORAMA): an international observational study}
        {The Lancet Oncology}
        {2025}
        {Alves et al. International paired non-inferiority study comparing AI and radiologists for pancreatic cancer detection on standard-of-care CT.}
    \vspace{1mm}
    \cventry
        {}
        {Navigating the landscape of multimodal AI in medicine: a scoping review}
        {Medical Image Analysis}
        {2025}
        {Schouten et al. Scoping review on technical challenges and clinical applications of multimodal AI in medicine.}
    \vspace{1mm}
    \cventry
        {}
        {Combining AI-based Biomarkers for PDAC Survival}
        {COMPAYL (MICCAI 2025 Workshop)}
        {Sep. 2025}
        {Hensens, Sabroso-Lasa, Verbeke, Malats, Litjens, \textbf{Vendittelli} et al. Multi-tissue segmentation pipeline combining TSR, mitosis density, stromal cell density and TTR for multivariate PDAC survival prediction.}

\end{cventries}
